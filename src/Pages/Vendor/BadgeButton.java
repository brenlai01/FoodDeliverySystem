...
public class BadgeButton extends JButton{
    ...
    private class BadgeButtonUI extends BasicButtonUI{
        ...
        @Override
        protected void paintText(Graphics graphics, JComponent jc, Rectangle rectangle, String string) {
            
        }
        
        @Override
        protected void paintIcon(Graphics graphics, JComponent jc, Rectangle IconRectangle) {
            if(image != null && !getText().equals((""))){
                Graphics2D g2 = (Graphics2D) graphics.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
                double size = Math.max(IconRectangle.getWidth(), IconRectangle.getHeight()) * 0.8f;
                double x = Math.min(IconRectangle.getX() + IconRectangle.getWidth() / 2, jc.getWidth() - size);
                double y = Math.max(IconRectangle.getY() - size / 2, 0);
                Area area = new Area(IconRectangle);
                area.subtract(new Area(new Ellipse2D.Double(x, y, size, size)));
                g2.setPaint(new TexturePaint(image, IconRectangle));
                g2.fill(area);
                String text = getText();
                createText(g2, x, y, size, text);
                g2.dispose();
            } else {
            super.paintIcon(graphics, jc, IconRectangle);
            }
        }
